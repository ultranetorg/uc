...
public struct ExpiredEC : IBinarySerializable, IEquatable<ExpiredEC>
{
	public Time Expiration;
	public long	Amount;

	public static long	Integrate(ExpiredEC[] x, Time expiration) => x?.SkipWhile(i => i.Expiration < expiration).Sum(i => i.Amount) ?? 0;

	public ExpiredEC()
	{
	}

	public ExpiredEC(Time expiration, long amount)
	{
	...
	public ExpiredEC Add(long y)
	{
		return new ExpiredEC(Expiration, Amount + y);
	}

	public static ExpiredEC[] Add(ExpiredEC[] x, ExpiredEC[] y)
	{
		var r = x;
	...
	public static ExpiredEC[] Subtract(ExpiredEC[] x, long y, Time time)
	{
		if(y == 0)	return [..x];
	...
	public static ExpiredEC[] Take(ExpiredEC[] x, long y, Time time)
	{
		if(y == 0)	return [..x];
		if(y < 0)	throw new ArgumentOutOfRangeException();
	...
	public static void Move(ref ExpiredEC[] from, ref ExpiredEC[] to, long y, Time time)
	{
		if(y == 0)	return;
		if(y < 0)	throw new ArgumentOutOfRangeException();
		...
		if(y - a > 0)
		{	
			to = Add(to, new ExpiredEC(from[old+full].Expiration, y - a));
	...
	{
		return obj is ExpiredEC eC && Equals(eC);
	}

	public bool Equals(ExpiredEC a)
	{
		return Expiration == a.Expiration && Amount == a.Amount;
	}

	public static bool operator ==(ExpiredEC left, ExpiredEC right)
	{

...
namespace Uccs.Rdn
{
	...
	public class RdnNode : McvNode
	{
		...
		public RdnNode(string name, Guid zoneid, string profile, RdnSettings settings, Vault vault, IEthereum ethereum, IClock clock, Flow flow) : base(name, settings ?? new RdnSettings(Path.Join(profile, zoneid.ToString())), vault, flow)
		{

...
CScreenRenderLayer * CRenderer::AddLayer(CScreenViewport * svp, CVisualGraph * g, CVisualSpace * space)
{
	auto l = new CScreenRenderLayer(Level);
	l->Viewport = svp;
	l->Graph = g->As<CDirectVisualGraph>();
	l->Space = space;
...
void CRenderer::RemoveLayer(CScreenRenderLayer * l)
{
...
void CRenderer::Update()
{
	...
	for(auto t : ScreenTargets)
	{
		...
		for(auto l : Layers)
		{
			...
			{
				l->Viewport->Apply();
				RenderSpace(t, l->Viewport, l->Graph, l->Space);
			}
		}
...
void CRenderer::RenderSpace(CRenderTarget * t, CViewport * vp, CVisualGraph * g, CVisualSpace * s)
{	

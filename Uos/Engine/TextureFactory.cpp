...
CSize CTextureFactory::GetDimension(CStream * s)
{
	auto size = s->GetSize();
	...
	CSize o(float(ilGetInteger(IL_IMAGE_WIDTH)), float(ilGetInteger(IL_IMAGE_HEIGHT)), float(ilGetInteger(IL_IMAGE_DEPTH)));

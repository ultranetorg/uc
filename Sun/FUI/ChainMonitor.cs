						...
						do 
						{
							...
							for(int i = last.Id - n + 1; i <= last.Id; i++)
							{
								...
								if(showt && r != null)
								{
									...
									nv = r.Id > Mcv.Pitch ? Math.Max(nv, IntLength((r.Majority != null ? r.Majority.Count() : 0)) + 1 + IntLength(r.RequiredVotes)) : 0;
								...
								if(r != null)
								{
									...
									if(showt)
									{
										var t = string.Format(	f, 
																...
																r.Try,
																r.Confirmed ? "c" : " ",
																r.Id > Mcv.Pitch ? $"{(r.Majority != null ? r.Majority.Count() : 0)}/{r.RequiredVotes}" : null,
																r.Members.Count,

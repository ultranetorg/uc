...
public class PublicKeyTests
{
 	...
 	public static void Main()
 	{
		...
		for(int i = 0; i < 100; i++)
		{
			...
			var x = new PublicKey(h);

			Assert.True(PublicKey.Parse(x.ToString()).Bytes.SequenceEqual(h));
			...
			x = new PublicKey(h, tag);

			var t = x.ToString();
			Assert.StartsWith(tag, t);

			var y = PublicKey.Parse(t);
	...
	[Fact]
 	public static void FormatAndAlgorithm()
 	{
		...
		var items = new List<PublicKey>();

		foreach(var f in Enum.GetValues<KeyFormat>())
		{
			foreach(var a in Enum.GetValues<Algorithm>())
			{
				items.Add(new PublicKey(f, a, h, null));
			}
		}

...
public static class PlatformExpressionTest
{
	...
	public static void General()
	{
		...
		{
			var p = new Platform {Family = Family.Windows};

			Assert.True(new Expression(Expression.Equal,			[new (Expression.Family), new ($"{Family.Windows}")]).Match(p));

			Assert.True(new Expression(Expression.Equal,			[new (Expression.Family), new ($"{Family.Windows}")]).Match(Platform.Current));
			Assert.False(new Expression(Expression.Equal,			[new (Expression.Family), new ($"{Family.Android}")]).Match(Platform.Current));
			
			Assert.True(new Expression(Expression.Equal,			[new (Expression.Brand), new ($"{Family.Windows}/{WindowsBrand.MicrosoftWindows}")]).Match(Platform.Current));
			Assert.False(new Expression(Expression.Equal,			[new (Expression.Brand), new ($"{Family.Windows}/{WindowsBrand.MicrosoftWindowsServer}")]).Match(Platform.Current));
			...
			Assert.True(new Expression(Expression.Equal,			[new (Expression.Architecture), new ($"{Architecture.x86_64}")]).Match(Platform.Current));
			Assert.False(new Expression(Expression.Equal,			[new (Expression.Architecture), new ($"{Architecture.x86_32}")]).Match(Platform.Current));

...
public class SeedHub
{
	...
	public IEnumerable<ReleaseDeclarationResult> ProcessIncoming(Endpoint ip, ResourceDeclaration[] resources)
	{
		foreach(var rsd in resources)
		{
			...
			lock(Mcv.Lock)
			{ 
				if(!Mcv.NextVotingRound.VotersRound.Members.OrderByHash(i => i.Address.Bytes, rzd.MemberOrderKey).Take(ResourceHub.MembersPerDeclaration).Any(i => Mcv.Settings.Generators.Any(g => g.Signer == i.Address)))
				{

...
const getSearchParams = (searchParams: URLSearchParams) => {
  ...
  const pageSize = isInteger(sizeParam) ? parseInt(sizeParam!) : DEFAULT_PAGE_SIZE_20
...
export const usePagePagination = () => {
  ...
  const [pageSize, setPageSize] = useState(DEFAULT_PAGE_SIZE_20)
  ...
  useEffect(() => {
    ...
    if (pageSize !== DEFAULT_PAGE_SIZE_20) {
  ...
  const resetPagination = useCallback(() => {
    setPage(0)
    setPageSize(DEFAULT_PAGE_SIZE_20)
    setSearch("")
  }, [])

...
export const FieldsTree = memo(({ items, onSelect, selected }: ProductFieldsTreeInternalProps) => {
  ...
  return (
    <>
      {items.map((node, index) => (
        <TreeNode key={node.id ?? index} node={node} selected={selected} onSelect={onSelect} />
      ))}
    </>
  )
})

...
const SlideContent = ({ item, isActive }: { item: SliderMediaItem; isActive: boolean }) => {
  ...
  switch (type) {
    ...
    case "video":
      return <VideoViewPlain url={item.src} className="size-full object-cover" controls={true} muted={true} posterUrl={item.poster} />
    case "youtube":
...
export const Slider = ({ items = [] }: SliderProps) => {
  if (!items.length) {
    return <div className="h-[416px] max-w-[750px] overflow-hidden rounded-xl bg-gradient-to-r from-[#0b1e3a] to-[#29593f] text-white" />
  }
  ...
  const thumbs = useMemo(() => {
    return items.map(item => {
      ...
      const thumbSrc =
        type === "youtube" ? inferPoster(item.src) ?? item.poster : item.poster ?? (isVideo ? inferPoster(item.src) : item.src)
            ...
            <button
              ...
              className={
                idx === activeIndex
                  ...
                  : "h-14 w-22 shrink-0 overflow-hidden rounded border-2 border-[#D7DDEB] outline-none focus:outline-none focus-visible:outline-none"
              }
            >

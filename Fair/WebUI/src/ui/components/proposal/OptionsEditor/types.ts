import { UseControllerProps } from "react-hook-form"

import { AccountBaseAvatar, CreateProposalData, OperationType } from "types"

export type FieldValueType =
  | "authors-additions"
  | "authors-removals"
  | "category"
  | "category-type"
  | "file"
  | "moderators-additions"
  | "moderators-removals"
  | "review-status"
  | "string"
  | "string-multiline"
  | "version"

export type FieldNameType =
  | "authorsIds"
  | "candidatesIds"
  | "categoryId"
  | "categoryTitle"
  | "description"
  | "fileId"
  | "moderatorsIds"
  | "nickname"
  | "parentCategoryId"
  | "siteTitle"
  | "slogan"
  | "status"
  | "type"
  | "version"

export type EditorField = {
  valueType?: FieldValueType
  name: FieldNameType
  placeholder?: string
  rules?: UseControllerProps<CreateProposalData>["rules"]
}

export type EditorFieldRendererParams = {
  errorMessage?: string
  field: EditorField
  value: string | string[] | AccountBaseAvatar[]
  onChange: (value?: string | string[] | AccountBaseAvatar[]) => void
}

export type EditorFieldRenderer = (params: EditorFieldRendererParams) => JSX.Element

export type ParameterValueType = "category" | "product" | "publication" | "review" | "user"

export type ParameterNameType = "categoryId" | "productId" | "publicationId" | "reviewId" | "userId"

export type EditorOperationFields = {
  operationType: Exclude<OperationType, "site-authors-change"> | "site-author-addition" | "site-author-removal"
  parameterValueType?: ParameterValueType
  parameterName?: ParameterNameType
  parameterLabel?: string
  parameterPlaceholder?: string
  parameterRules?: UseControllerProps<CreateProposalData>["rules"]
  fields?: EditorField[]
}

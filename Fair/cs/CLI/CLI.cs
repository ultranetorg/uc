	...
	public class Program
	{
		...
		public object Execute(IEnumerable<Xon> command, Flow flow)
		{
			...
			if(command.Skip(1).FirstOrDefault()?.Name == "?")
			{
				...
				var t = GetType().Assembly.GetTypes().FirstOrDefault(i => i.BaseType == typeof(FairCommand) && i.Name.ToLower() == command.First().Name + "command");

				var c = Activator.CreateInstance(t, [this, null, flow]) as FairCommand;
			...
			else
			{
				...
				if(r is Operation o)
				{
					...
					{
						var txs = c.Transact([o], c.GetAccountAddress("signer"), FairCommand.GetAwaitStage(command));

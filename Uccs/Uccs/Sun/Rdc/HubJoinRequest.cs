...
namespace Uccs.Net
{
	public class HubJoinRequest : RdcRequest
	{
			...
			lock(core.Lock)
			{
				...
				else if(core.Synchronization == Synchronization.Synchronized)
				{
					...
					for(int i = RoundId; i > RoundId - Database.Pitch * 2; i--) /// not more than 1 request per [2 x Pitch] rounds
						if(d.FindRound(i) is Round r && r.HubJoinRequests.Any(j => j.Account == Account))
							return null;
				
					core.Database.GetRound(RoundId).HubJoinRequests.Add(this);
				}
	
				foreach(var i in core.Connections.Where(i => i != Peer))
				{
					i.Send(new HubJoinRequest {RoundId = RoundId, Signature = Signature});
				}
			}

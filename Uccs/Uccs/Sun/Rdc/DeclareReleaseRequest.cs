...
namespace Uccs.Net
{
	...
	{
		public Dictionary<ResourceAddress, Availability>	Packages { get; set; }
		...
		public override RdcResponse Execute(Core core)
		{
			...
			core.Seedbase.Add(Peer.IP, Packages);
		...
		{
			var aa = Packages.GroupBy(i => i.Key.Author);
		...
		{
			Packages = new Dictionary<ResourceAddress, Availability>();
			...
			for(int i=0; i<an; i++)
			{
				...
				for(int j=0; j<rn; j++)
				{
					...
					Packages[new ResourceAddress(a, r)] = (Availability)reader.ReadByte();
				}
			}

					...
					do 
					{
						...
						for(int i = last.Id - n + 1; i <= last.Id; i++)
						{
							...
							if(showt && r != null)
							{
								...
								nv = r.Id >= Mcv.P ? Math.Max(nv, IntLength(r.SelectedArrived.Count()) + 1 + IntLength(r.MinimumForConsensus)) : 0;
							...
							if(r != null)
							{
								...
								if(showt)
								{
									var t = string.Format(	f, 
															...
															r.Try,
															r.Confirmed ? "c" : " ",
															r.Id >= Mcv.P ? $"{r.SelectedArrived.Count()}/{r.MinimumForConsensus}" : null,
															r.Members?.Count,

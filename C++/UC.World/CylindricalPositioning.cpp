	...
	PlaceRandom =	[this](auto c, auto pp, auto & pk, auto & s)
					{
						...
						return CMatrix::FromTransformation(	CFloat3(0, p.y, R),
	...
	PlaceCenter =	[this](auto c, auto pp, auto & pk, auto & s)
					{
						...
						return CMatrix::FromTransformation(	CFloat3(0, 0, R),
															CQuaternion(0, CFloat3(0, 0, 1).GetAngleInXZ(p), 0),
															CFloat3(0, 0, -R),
															CFloat3(1)).Decompose();
					};
	...
	PlaceExact =	[this](auto c, auto pp, auto & pk, auto & s)
					{
						...
						return CMatrix::FromTransformation({0, p.y, R},
														   {0, CFloat3(0, 0, 1).GetAngleInXZ(p), 0},
														   {0, 0, -R},
														   CFloat3(1)).Decompose();
					};
}

CTransformation CCylindricalPositioning::Adjust(CTransformation & t, CTransformation & d)
{
	//return (CMatrix(t) * (CMatrix(t).GetInversed() * CMatrix(d) * CMatrix(t))).Decompose();
	return t;
}
...
CFloat3 CCylindricalPositioning::GetDirection(CViewport * vp, CActive * node, CActiveSpace * s, CSize & size, CTransformation & t)
{
	...
	return (CMatrix(t) * s->Matrix).GetPosition() * im;
}
...
CPositioningCapture CCylindricalPositioning::Capture(CPick & pick, CSize & size, CMatrix & wm)
{
	...
	c.Offset = (wm * im).GetPosition();

	c.Angle = (wm * im).GetPosition().GetAngleInXZ(CFloat3(0, 0, 1));
...
CTransformation CCylindricalPositioning::Move(CPositioningCapture & c, CPick & pick)
{
	...
	auto m = CMatrix::FromTransformation(CFloat3(0, 0, R) + CFloat3(0, c.Offset.y, 0) + CFloat3(0, p.y - c.Point.y, 0), 

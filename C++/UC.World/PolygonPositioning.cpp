...
CPolygonalPositioning::CPolygonalPositioning(bool leftbottom)
{
		...
		PlaceRandom =	[this](auto c, auto pp, auto & pk, auto & s)
						{
							...
							return CTransformation(p.x, p.y, 0);
						};
...
CTransformation CPolygonalPositioning::Adjust(CTransformation & t, CTransformation & d)
{
...
CTransformation CPolygonalPositioning::Move(CPositioningCapture & c, CPick & pick)
{
	...
	if(!LeftBottom)
	{
		if(!CPolygon::Contains(b, CFloat2(p.x - c.Size.W/2, p.y)))
		{
			p = bound(p + CFloat3(-c.Size.W/2, 0, 0));
			p.x += c.Size.W/2;
		}
	
		if(!CPolygon::Contains(b, CFloat2(p.x, p.y - c.Size.H/2)))
		{
			p = bound(p + CFloat3(0, -c.Size.H/2, 0));
			p.y += c.Size.H/2;
		}

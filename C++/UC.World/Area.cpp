...
void CArea::Save()
{
	CTonDocument d;
	Save(&d);
	auto f = Level->Storage->WriteFile(CPath::Join(IFileSystem::UserGlobal, Directory, GetClassName() + L".area"));
...
void CArea::Load()
{
	auto s = Level->Storage->ReadFile(CPath::Join(IFileSystem::UserGlobal, Directory, GetClassName() + L".area"));

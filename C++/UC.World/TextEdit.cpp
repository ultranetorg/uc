...
void CTextEdit::OnStateChanged(CActive * s, CActive *, CActiveStateArgs * a)
{
	...
	if(!Caret->Parent && a->New == EActiveState::Active)
	{
		Visual->AddNode(Caret);
		Level->Core->AddWorker(this);
	}
	if(Caret->Parent && a->Old == EActiveState::Active)
	{
...
{
	if(a->Control == EMouseControl::Wheel && a->Action == EMouseAction::Rotation)
	{
	...
	if(a->Control == EMouseControl::LeftButton && a->Action == EMouseAction::On)
	{
		auto p = a->Pick.Point;
...
void CTextEdit::OnKeyboard(CActive *, CActive *, CKeyboardArgs * a)
{
	...
	if(a->Action == EKeyboardAction::On)
	{
...
void CTextEdit::UpdateCaret()
{
	...
	for(int i=0; i<CaretPos.x; i++)
	{
		if(Lines[CaretPos.y][i] != L'\t')
			x += int(Font->Widths[Lines[CaretPos.y][i]]);
		else
		{
			if(x % int(t) == 0)
				x += int(t);
...
void CTextEdit::Draw()
{
	if(Font)
	{
		...
		auto y = IsMultiLine ? c->H : c->H - (c->H - Font->Height)/2;

		while(line != Lines.end() && y + Font->Height > 0)
		{
			c->DrawText(*line, Font, CFloat4(1), CRect(-Xscroll, y - Font->Height, FLT_MAX, Font->Height), EXAlign::Left, EYAlign::Center, false, false);

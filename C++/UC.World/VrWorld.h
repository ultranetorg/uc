	...
	class CVrWorld : public CDesktopWorld
	{
		public:
			...
			void InitializeAreas() override
			{
				...
				MainArea->MaxSize =	 [](auto vp){ return CSize(vp->W - 50, vp->H - 50, FLT_MAX); };
				FieldArea->MaxSize = [](auto vp){ return CSize(vp->W - 50, vp->H - 50, FLT_MAX); };
				...
				NearArea->PlaceNewDefault =	[](auto c, auto pp, auto & pk, auto & s)
											{
												return CTransformation(0, 2000, 0);
											};
			}
			...
			void OnMouse(CActive * r, CActive * s, CMouseArgs * arg) override
			{
				if(	arg->Capture && 
					...
					arg->Capture->InputAs<CMouseInput>()->Control == EMouseControl::MiddleButton && 
				   (arg->Event == EGraphEvent::Hover || arg->Event == EGraphEvent::Roaming))
				{
					auto w = MainView->PrimaryCamera->Viewport->W;
					auto h = MainView->PrimaryCamera->Viewport->H;
		
					auto d = arg->PositionDelta;
			...
			void DirectCameras(CFloat3 & d)
			{
				...
				Core->DoUrgent(this, L"Main camera directing",	[this]
																{ 
				...
				Core->DoUrgent(this, L"Near camera directing",	[this]
																{ 

...
void CEnvironmentWindow::OnMouse(CActive * r, CActive * s, CMouseArgs * a)
{
	if(a->Event == EGraphEvent::Click)
	{
		...
		if(a->Control == EMouseControl::RightButton)
		{
			if(!ServiceMenu)
			{
...
{
	if(a->Event == EGraphEvent::Captured && a->Capture->Message->As<CMouseInput>()->Control == EMouseControl::RightButton)
	{
		auto u = Url;
	...
	Sizer->Adjusting =	[this](CResizing & r)
						{
							...
							GetUnit()->AdjustTransformation(CTransformation(r.PositionDelta/2.f));
						};
}
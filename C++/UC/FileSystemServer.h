...
namespace uc
{
	class UOS_LINKING CFileSystemServer : public CServer, public CFileSystem, public IExecutor
	{	
		public:
			struct CMapping
			{
			...
			UOS_RTTI
			CFileSystemServer(CNexus * l, CServerInstance * info);
			~CFileSystemServer();
	...
	{
		public:
			CFileSystemServer * Server;

			UOS_RTTI
			CFileSystemClient(CNexus * nexus, CClientInstance * instance, CFileSystemServer * server) : CClient(instance)
			{
				Server = server;
			}

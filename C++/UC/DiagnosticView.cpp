...
CDiagnosticView::CDiagnosticView(CCore * e, CDiagnostic * diag, CGdiRect & r, CNativeWindow * mmc, int menuItemID) : 
					CNativeWindow(e, 0, L"", WS_VISIBLE|WS_SYSMENU|WS_THICKFRAME, (int)r.X, (int)r.Y, (int)r.W, (int)r.H, mmc->Hwnd, null, e->Core->LocationInstance, null)
{
	Core		= e;
	Mmc			= mmc;
...
bool CDiagnosticView::ProcessMessage(HWND hwnd, UINT msg, WPARAM w, LPARAM l, INT_PTR * r)
{
	...
	switch(msg)
	{
		...
		case WM_MOUSEWHEEL:
			...
			UpdateData.Count = RectClient.H/CharHeight + 1;
			Update();
			return false;
...
void CDiagnosticView::ResizeBitmaps()
{
	...
	MemBitmap = CreateCompatibleBitmap(DC, (int)RectClient.W, (int)RectClient.H);
...
void CDiagnosticView::OnAdded(CString & t, DWORD c)
{
	...
	if(UpdateData.Offset <= Diag->Lines && Diag->Lines < UpdateData.Offset + UpdateData.Count)
	{
		...
		if(y <= Highlighted.y && Highlighted.y < y + CharHeight && Highlighted.x > 0 && Highlighted.x < RectClient.W)
		{

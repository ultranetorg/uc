...
CString CStorage::UniversalToNative(CString const & p)
{
	...
	{
		path = Level->Core->GetPathTo(ESystemPath::Root, CPath::Nativize(lpath));
	}
...
{
	return addr.StartsWith(L"/" UOS_MOUNT_SERVER L"/") || addr.StartsWith(L"/" UOS_MOUNT_CORE L"/");
}

CString CStorage::RemapToCommon(CString const & u)
{
	CString s;

	if(u.StartsWith(L"/" UOS_MOUNT_CORE L"/"))		s = UniversalToNative(L"/" UOS_MOUNT_CORE L"/"); else
	if(u.StartsWith(L"/" UOS_MOUNT_SERVER L"/"))	s = UniversalToNative(L"/" UOS_MOUNT_SERVER L"/");

	auto p = UniversalToNative(u).Substring(s.size());

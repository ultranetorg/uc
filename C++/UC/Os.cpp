...
CVersion COs::GetVersion()
{
	return Version;
}

void COs::SaveDCToFile(LPCTSTR FileName, HDC hsourcedc, int Width, int Height)
{
	if(!FileName) return;
...
CList<CString> COs::OpenFileDialog(DWORD options, CArray<std::pair<CString,CString>> & types)
{
	CList<CString> o;
...
BOOL COs::SetPrivilege(HANDLE hToken, LPCTSTR lpszPrivilege, BOOL bEnablePrivilege)
{
	TOKEN_PRIVILEGES tp;
	LUID luid;
...
CString COs::GetEnvironmentValue(CString const & name)
{
	wchar_t b[32767];

	if(GetEnvironmentVariable(name.data(), b, _countof(b)) > 0)
	{
		return b;
	}
	else

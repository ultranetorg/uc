...
CMmc::CMmc(CCore * core) : CNativeWindow(core, core->LocationInstance, IDF_MMC, null)
{
	Core = core;
	...
	if(Config->Get<CString>(L"State") == L"Visible")
	{
		View = new CLogView(Core, Core->Log, GetDlgItem(GetHwnd(), IDC_MMCVIEW));
	
		DeactivationAction	= Config->One(L"DeactivationAction")->AsString();
			
		if(Config->One(L"StartSize")->Get<CString>() == L"Default")
		{
			SetSize(640, 320);
		}
		else if(Config->One(L"StartSize")->Get<CString>() == L"Custom")
		{
			CGdiRect r = Config->One(L"CustomRect")->Get<CGdiRect>();
			SetSize(r.W, r.H);
		}
	
		if(Config->One(L"StartPosition")->Get<CString>() == L"Default")
		{
			RECT pd;
			SystemParametersInfo(SPI_GETWORKAREA, 0, &pd, 0);
			CGdiRect d = pd;
			CGdiRect r = GetRect();
			SetPosition((d.W - r.W)/2, (d.H-r.H)/2);
		}
		else if(Config->One(L"StartPosition")->Get<CString>() == L"Custom")
		{
			auto  r = Config->One(L"CustomRect")->Get<CGdiRect>();
			SetPosition(r.X, r.Y);
		}
		...
		Core->ExitRequested		+= ThisHandler(OnExitQueried);
...
void CMmc::OnSize()
{
	...
	if(View != null)
	{
		View->SetRect(CGdiRect(0, 0, rr.W, rr.H));
	}
}
...
void CMmc::AddLog(CLog * log)
{
	...
	for(auto i : Config->Many(L"LogView"))
	{
...
void CMmc::AddDiag(CDiagnostic * diag)
{
	...
	for(auto i : Config->Many(L"DiagView"))
	{
...
void CMmc::ShowLog(CMmcMenuItem * i, bool show)
{
	if(show && i->View == null)
	{
		if(i->Parameter == null)
		{
			i->Parameter = Config->Add(L"LogView");
		}
...
void CMmc::ShowDiag(CMmcMenuItem * i, bool show)
{
	if(show && i->View == null)
	{
		if(i->Parameter == null)
		{
			i->Parameter = Config->Add(L"DiagView");
		}

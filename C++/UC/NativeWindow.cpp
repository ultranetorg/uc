...
{	
	SetWindowPos(Hwnd, Hwnd, r.X, r.Y, r.W, r.H, SWP_NOZORDER);
}

void CNativeWindow::SetPosition(int x, int y)
{	
	CGdiRect r = GetRect();
	MoveWindow(Hwnd, x, y, r.W, r.H, false);
}
...
void CNativeWindow::SetOwner(CNativeWindow * o)
{
	SetWindowLongPtr(Hwnd, GWLP_HWNDPARENT, (LONG_PTR)o->Hwnd);
}

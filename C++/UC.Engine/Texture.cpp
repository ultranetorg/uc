...
bool CTexture::Open(HANDLE shared)
{
	ID3D11Texture2D * t = nullptr;
		
	if(GraphicEngine->Devices.front()->DxDevice->OpenSharedResource(shared, __uuidof(ID3D11Texture2D), (void**)(&t)) == S_OK)
	{
		Shared = shared;
...
CCanvas * CTexture::BeginDraw(CFloat2 & s)
{
	auto t = DxTextures.begin();
	Canvas = new CCanvas(Level, GraphicEngine, t->first, t->second, s);
	return Canvas;
